...
import ImageSlider from '@/app/(base)/items/[url]/ImageSlider';
const DataCard = ({ data }) => {
    if (data.category == "laptop") return <Laptop laptop={data}></Laptop>
    else return <General data={data}></General>
    ...
    return (
        <div className="group bg-gray-50 border-2 rounded flex gap-1 py-5 items-center justify-between flex-col w-full relative">
            ...
            <div className=' w-full p-6 lg:p-3   relative overflow-hidden '>
                ...
                <Link className='text-sm font-semibold px-2 text-blue-700 capitalize hover:underline underline-offset-4 py-2' href={"items/" + laptop.dataUrl}><Image alt={laptop.dataUrl} height={125} width={220} src={laptop.images[0]} className=' aspect-video w-full object-cover bg-white rounded border-transparent outline-transparent duration-100  z-30  hover:scale-110'></Image></Link>
            ...
            <Link className='text-sm font-semibold px-2 text-blue-700 capitalize hover:underline underline-offset-4 py-2' href={"items/" + laptop.dataUrl}>{laptop.laptop.brand + " " + laptop.laptop.model}</Link>
...
const General = ({ data }) => {
    ...
    if (Number(data?.stock) <= 0) flag = <div className='absolute top-0 rounded-tl-sm rounded-br-md left-0 bg-red-500 text-white px-2 text-xs font-semibold'>Out of Stock</div>
    else if (Number(data?.stock) < 10) flag = <div className='absolute top-0 rounded-tl-sm rounded-br-md left-0 bg-orange-500 text-white px-2 text-xs font-semibold'>Low Stock</div>

    return (
        <div className="group bg-gray-50 border-2 rounded flex gap-1 py-5 items-center justify-between flex-col w-full relative">
            {/* preview image slider for cards */}
            {show && <div className="w-full  z-[51] h-screen fixed top-0 left-0 flex items-center justify-center">
                <div className="relative w-4/5 lg:w-2/5 aspect-video bg-white z-50">
                    <RxCross2 onClick={() => setShow(false)} className='absolute top-2 right-2 text-2xl cursor-pointer z-50 bg-white rounded' />
                    <ImageSlider images={images}></ImageSlider>
            ...
            <div className=' w-full p-6 lg:p-3   relative overflow-hidden '>
                ...
                <Link className='text-sm font-semibold px-2 text-blue-700 capitalize hover:underline underline-offset-4 py-2' href={"items/" + data?.dataUrl}><Image alt={data?.dataUrl} height={125} width={220} src={data?.images[0]} className=' aspect-video w-full object-cover bg-white rounded border-transparent outline-transparent duration-100  z-30  hover:scale-110'></Image></Link>
                <div className='absolute lg:-right-10 lg:group-hover:right-0 right-0  top-0  h-full w-10 duration-100 bg-gray-200/50 backdrop-blur-lg lg:bg-gray-200 z-50 flex items-center justify-center gap-1 flex-col overflow-hidden'>
                    <CartButton id={data?._id}></CartButton>
                    <Image onClick={() => setShow(true)} height={10} width={30} src={eye} alt='preview' className='bg-white p-2 cursor-pointer active:scale-90 duration-100 hover:bg-blue-500' title='Preview'></Image>
                </div>

            </div>
            <div className='flex justify-between items-center flex-col grow shrink-0 w-full px-2 text-justify'>

                <Link className='text-sm font-semibold px-2 text-blue-700 capitalize hover:underline underline-offset-4 py-2' href={"items/" + data?.dataUrl}>{data?.title}</Link>
                <p className=' text-xs font-semibold text-gray-700'> <span className='text-black font-semibold rounded'>Description: </span>{data?.description.slice(0,110)}...</p>

                <p className=' text-sm pt-3 font-semibold '> <span className='text-black px-1 rounded'>Price :</span> {data?.price} ৳ </p>

            </div>
            <OrderBtn laptop={data}></OrderBtn>
        </div>
    );
};
