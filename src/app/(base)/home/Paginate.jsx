...
const PaginateItem = ({ length, current, request }) => {
    ...
    let button = <Link href={`/search?page=${current}`} className={`${current == request ? "bg-blue-400 text-white border-blue-400" : "bg-white"} border-2 border-black font-semibold px-3 py-1 rounded  active:scale-90 duration-100`}>{current + 1}</Link>
