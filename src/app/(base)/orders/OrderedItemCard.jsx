...
const OrderedItemCard = async ({ id, quantity, price }) => {
    try {
        ...
        data = data.result;
        if (data.category == "laptop") return Laptop(data, quantity, price);
        return General(data, quantity, price);
    } catch (error) {
        console.log(error);
...
const Laptop = (data, quantity, price) => {
...
const General = (data, quantity, price) => {
    console.log("general");
    return <div className=" rounded flex flex-col lg:flex-row gap-1 py-2 lg:pr-6 my-2 items-center lg:items-center justify-between ">
        <div className='flex flex-col md:flex-row gap-2 items-center '>
            <div className='w-40 px-2 group overflow-hidden'>
