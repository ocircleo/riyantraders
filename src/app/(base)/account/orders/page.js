...
const Page = () => {
  ...
  useEffect(() => {
    (async function () {
      ...
      if (temUser) {
        try {
          const res = await fetch(API + "order/myOrders/" + temUser.phone, {
            method: "GET",
            headers: {
  ...
  return (
    <div>
      ...
      <div className="flex flex-col gap-2">
        <div className="shadow-inner border p-2 sm:bg-green-200 hidden sm:grid grid-cols-8 w-full font-semibold">
          <div>SI </div>
          <div>Status</div>
          <div>Detail</div>
